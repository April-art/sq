<template>
    <div>
        <v-touch @swipeleft="swiperleft" :swipe-options="{direction: 'horizontal'}"  @swiperight="swiperright">
            <div class="Sunday_cont">
                <div class="Sunday_content">
                        <router-link v-for="(data,index) in obj"  :key="index" class="Sunday_item" :to="{path:'/CTcontent/Introduction',query:{url:data.url}}">
...
export default {
    ...
    mounted(){
        let that = this
        this.$axios.get('http://api.pingcc.cn/?mhname=的',{
            parmas:{

            }
       }).then(res=>{
           that.obj = res.data.list.slice(0,10)            
    ...
    methods:{
        ...
        swiperleft:function(){
            this.$router.push({'path':'/buttom/classify_header/weeks/finish'})
        },
