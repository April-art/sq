    ...
    <div class="vuebody">
        ...
        <div class="page">
            <van-row gutter="20" style="margin-right:0px">
                <div id="activeClass">
                <van-col span="9"  v-for="(key,index) in listone1" :key="index" @click="GetListImgs(key.url)"   style="text-align:center;padding:15px;margin:20px 0px 0px 30px;">{{key.num}}</van-col>
                </div>
            </van-row>
        </div>
    ...
    methods:{
        GetListImgs(a){
            ...
            if(a==undefined){
                ...
                // $('.mint-tabbar.is-fixed').css('display','none')
        })
            }
        ...
        GetListImg1(){
            ...
            that.$axios({
                methods:"get",
                //接口地址  api-代理+接口端口号之后的其余地址
                url:"http://api.pingcc.cn/?mhurl1=wbmh71462",
                headers: {
                "Content-Type": "application/x-www-form-urlencoded",
               }
            }).then(response =>{
